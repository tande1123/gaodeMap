 ...
 * @LastEditTime: 2019-08-09 15:15:14
 -->
...
<template>
  <div>
    ...
    <el-scrollbar class="childThree" style="height:100%">
      <ul class="sidebar-menu">
        <li v-for="(item,index) in listThree" :key="index">
          <p>{{item.STNM}}</p>
          <p>{{item.DRP}}mm</p>
          <div :class="getclass(item.DRP)"></div>
        </li>
      </ul>
    </el-scrollbar>
  </div>
...
export default {
  ...
  computed: {},
  methods: {
    getclass (data) { // 判断data值并返回对应class名
      if (data < 0.1) {
        return 'color1'
      }
      if (data >= 0.1 && data < 10) {
    ...
    initData () {
      ...
      this.axios({// 获取雨情监测点数据
        ...
        that.listThree = common.convertTable2objects(res.data.QuerySafetyResult)
      })
    }
  },
  watch: {},
